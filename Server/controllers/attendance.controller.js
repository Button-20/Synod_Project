  ...
  if (
    ...
    attendance.save((err, doc) => {
      if (!err) res.send(doc);
        ...
        if (err.code == 11000) res.status(422).send(["Duplicate Date found."]);
...
module.exports.get = (req, res) => {
  Attendance.find((err, docs) => {
    if (!err) {
      ...
      console.log(
        "Error in retrieving Attendance :" + JSON.stringify(err, undefined, 2)
      );
    }
  });
};
...
module.exports.getAllCount = (req, res) => {
  Attendance.countDocuments({}, (err, docs) => {
    if (!err) {
      ...
      console.log(
        "Error in retrieving Attendance Count :" +
  ...
  Attendance.find(
    ...
    (err, doc) => {
      if (!err) {
        ...
        console.log(
          "Error in Retrieving Attendance with Date :" +
            JSON.stringify(err, undefined, 2)
        );
      }
    }
...
module.exports.getID = (req, res) => {
  if (!ObjectId.isValid(req.params.id))
    return res
      .status(400)
      .send(`No Attendance found with given id : ${req.params.id}`);
...
module.exports.getClassname = (req, res) => {
  Attendance.find({ classname: req.params.classname }, (err, doc) => {
    if (!err) {
      ...
      console.log(
        "Error in Retrieving Attendance with Classname :" +
          JSON.stringify(err, undefined, 2)
      );
    }
  });
  ...
  Attendance.findByIdAndUpdate(
    ...
    (err, doc) => {
      if (!err) {
        ...
        console.log(
          "Error in Attendance Update :" + JSON.stringify(err, undefined, 2)
        );
      }
    }
  );
